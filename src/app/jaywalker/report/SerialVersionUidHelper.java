...
public class SerialVersionUidHelper {
    ...
    private static class FieldOrMethodComparator implements Comparator {
        public int compare(Object o1, Object o2) {
            ...
            int compareValue = fieldOrMethod1.getName().compareTo(fieldOrMethod2.getName());
            if (compareValue == 0) {
                compareValue = fieldOrMethod1.getSignature().compareTo(fieldOrMethod2.getSignature());
            }
            return compareValue;
        }
    }
...
try {
        ...
        if (isSerializable != null) return toBoolean(isSerializable);

        isUrlSerializableMap.put(url, SERIALIZABLE_PENDING);
        ClassElement classElement = (ClassElement) factory.create(url);
        return isSerializable(classElement);} catch (IllegalStateException e) {
    ...
    private boolean isSuperSerializable(ClassElement classElement) {
        ...
        if (superclassUrls == null || superclassUrls.length == 0) {
            ...
            // If class resolves to multiple super classes, then find at least one that is serializable
            // This is acceptable since the serialVersionUid is not calculated using the super's state
    ...
    private boolean isSerializable(String className) {
        String isSerializable = (String) isClassNameSerializableMap.get(className);
        if (isSerializable != null) return toBoolean(isSerializable);
    ...
    private void flagAsNotSerializable(JavaClass javaClass) {
        isClassNameSerializableMap.put(javaClass.getClassName(), SERIALIZABLE_NO);
    }

    private void flagAsSerializable(JavaClass javaClass) {
        isClassNameSerializableMap.put(javaClass.getClassName(), SERIALIZABLE_YES);
    }
    ...
    private boolean toBoolean(final String serializable) {
        if (serializable == SERIALIZABLE_PENDING) {
            throw new IllegalStateException("Should not be in a pending state after isSerializable method call.");
        } else {
     ...
     */
    private long computeSerialVersionUID(JavaClass javaClass) throws IOException, NoSuchAlgorithmException {
        ...
        classModifiers &= (Modifier.PUBLIC | Modifier.FINAL | Modifier.INTERFACE | Modifier.ABSTRACT);

        // compensate for javac bug in which ABSTRACT bit was set for an
        // interface only if the interface declared methods
        if (javaClass.isInterface()) {
            classModifiers = (methods.length > 0)
                    ? (classModifiers | Modifier.ABSTRACT) : (classModifiers & ~Modifier.ABSTRACT);
        }
    ...
    public boolean isSerialVersionUidsConflicting(URL[] urls) {
        ...
        if (urls.length > 0) {
            if (!isSerializable(urls[0])) return false;
        ...
        for (int i = 1; i < urls.length; i++) {
            if (!isSerializable(urls[i])) return false;
